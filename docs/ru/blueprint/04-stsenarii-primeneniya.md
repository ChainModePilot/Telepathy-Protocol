...
**Сценарий**: Human Prime-пациент нуждается в том, чтобы его медицинский Fay передал данные о здоровье страховому coFay для получения оценки страхового случая.
...
5. **Полный аудит**: Все записи о доступе к данным фиксируются в аудиторском следе, и Human Prime-пациент может просмотреть их в любое время

```mermaid
sequenceDiagram
    participant H as Human Prime-пациент
    ...
    Note over H: Human Prime может аудировать все записи доступа
```

Этот сценарий воплощает принцип TP **суверенитета Human Prime над конфиденциальностью** — объём раскрытия данных всегда определяется Human Prime, а не собственным суждением Fay.
...
**Сценарий**: Юридический Fay (представляющий Human Prime-клиента) инициирует консультацию с налоговым coFay, нуждаясь в получении налоговых записей клиента для поддержки подготовки к судебному разбирательству.
...
1. **Делегирование Human Prime**: Human Prime-клиент авторизует юридического Fay через протокол FP, разрешая ему получать налоговые записи от его имени
...
6. **Аудиторский след**: Все записи об использовании credentials и доступе к данным записываются в журнал аудита, и Human Prime-клиент может просмотреть их в любое время

```mermaid
sequenceDiagram
    participant C as Human Prime-клиент
...
Этот сценарий демонстрирует, как TP оцифровывает реальный паттерн «делегат ведёт дела по доверенности» — credentials ограничены по времени, имеют определённый объём, отзываемы и аудируемы, полностью защищая интересы Human Prime.
