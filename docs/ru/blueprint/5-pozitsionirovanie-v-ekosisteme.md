...
| **Атрибуция идентичности** | Отсутствует (инструменты не имеют понятия атрибуции) | Отсутствует (Agents — автономные сервисные узлы) | Да (каждый Fay действует от имени Host) |
| **Защита конфиденциальности** | Нет систематического механизма (передача параметров открытым текстом) | Нет систематического механизма | Сквозное шифрование + Выборочное раскрытие + Авторизация Host |
