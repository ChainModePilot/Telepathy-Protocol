...
Phase 2 深入安全和隐私领域。主要里程碑包括：交付加密和凭证相关的 Schema 定义（EncryptedPayload、CallbackCredential）；发布安全规范，涵盖端到端加密、认证机制和宿主委托授权；定义共享语境的完整生命周期管理——创建、范围界定、过期和撤销；建立 Technical Steering Committee（TSC）治理机制。Phase 2 的准出标准是安全 Schema 通过一致性校验，且 TSC 章程正式发布。
